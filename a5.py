...
# ------------------------------
# Ratios for different spherical aberration coefficients
ratios = {}
for Cs_mm in [5, 10, 50]:
    Cs = Cs_mm * 1e-3
...
for i, (Cs_mm, ratio) in enumerate(ratios.items()):
    ax.fill_between(alpha_mrad, ratio, alpha=0.25, color=colors[i])
    ax.plot(alpha_mrad, ratio, color=colors[i], linewidth=2, label=f"Cs = {Cs_mm} mm")

# Labels & title
ax.set_xlabel("Semi-convergence angle α (mrad)", fontsize=12)
ax.set_ylabel("Spherical aberration contribution (%)", fontsize=12)
ax.set_title("Spherical Aberration Contribution — Shaded Style", fontsize=14, weight='bold')
